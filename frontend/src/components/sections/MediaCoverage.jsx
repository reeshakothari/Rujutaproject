...
export default function MediaCoverage() {
  ...
  const m = t.media;
  return (
    <section id="media" data-testid="media-section" className="scroll-mt-20 overflow-x-hidden bg-rutuja-soft py-16 md:py-32">
        ...
        <div className="mt-14 grid grid-cols-1 gap-8 md:grid-cols-3">
          {m.items.map((item, i) => (
            <Reveal key={i} delay={i * 0.08}>
              ...
              >
                <FlyIn direction={i % 2 === 0 ? "left" : "right"} className="relative overflow-hidden border border-rutuja-line bg-white shadow-[0_20px_50px_-30px_rgba(0,0,0,0.5)] transition-[transform,box-shadow,border-color] duration-500 group-hover:-translate-y-1.5 group-hover:border-rutuja-pink/40 group-hover:shadow-[0_25px_60px_-24px_rgba(200,43,98,0.55)]">
                  <div className="aspect-[4/3] overflow-hidden">
                    <img
                      src={MEDIA[i]}
                      alt={`${item.publication} — ${item.headline}`}
                      className="h-full w-full object-cover object-top transition-transform duration-[400ms] ease-out group-hover:scale-[1.04]"
                      loading="lazy"
                    />
                  </div>
                  <span className="absolute right-3 top-3 grid h-9 w-9 place-items-center bg-white/90 text-rutuja-blue opacity-0 backdrop-blur-sm transition-opacity duration-300 group-hover:opacity-100">
                    <ExternalLink size={16} className="animate-icon-glow-blue" />
                  </span>
                </FlyIn>
                ...
                <span className="mt-4 inline-flex items-center gap-1.5 text-xs font-semibold uppercase tracking-[0.15em] text-rutuja-pink">
                  {m.cta} <ExternalLink size={13} className="animate-icon-glow" />
                </span>
              </a>
            </Reveal>
          ))}
