...
- Impact revert to pink + numbers move (2026-08-21): Per user request — (1) reverted the impact map section from the editorial ivory/burgundy/gold back to the site's PINK UI (rutuja tokens: light-pink #F0DEE6 map fill, pink #C82B62 markers/journey lines, pink story card); removed gold block-print motifs. (2) Moved the "Documented field activity" story card to OVERLAY the map's bottom-right on desktop (stacks below the map on mobile). (3) Changed the ImpactMetrics "Impact" section (was ambassador 20+/700+/6) to the Doll Pilot Project numbers (2,924 / 38 / 13 / 11 May–31 Jul 2026 / 30–60 min) and REMOVED the now-duplicate National Pilot strip from the map section (single source of the pilot numbers). /impact page now renders ImpactMetrics (pilot numbers) + ImpactMap. Verified via screenshots on desktop + mobile (390px, 0 horizontal overflow), region interaction + count-up working.
