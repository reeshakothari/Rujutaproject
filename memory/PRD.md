...
- Editorial Impact redesign (2026-08-21): Rebuilt the impact map into a premium editorial section "Our Impact / Where the Dignity Doll Has Reached" per design brief + design_guidelines.json. Structure: eyebrow 'WHERE THE CONVERSATION IS HAPPENING' + headline 'From one conversation to communities across India.' + subtext; India SVG map with 6 STATE-level region markers (refined glow/pulse, no bounce) revealing sequentially, gold bezier journey lines from the Mumbai hub, block-print vector motifs (corner flourish, kalka divider, lotus watermark); region selector (vertical list desktop / horizontal pills mobile) + editorial story card (city, documented context, venue chips; per-city PHOTOS intentionally omitted for data-safety — no approved attribution). DATASET SEPARATION: a distinct 'The National Pilot — By the Numbers' strip shows EXACT pilot figures 2,924 females reached / 38 sessions / 13 participants / 11 May–31 Jul 2026 window / 30–60 min length, explicitly labelled and kept separate from the older ambassador field-report metrics (20+/700+/6 in the ImpactMetrics section). Closing dark-burgundy quote card + CTAs (workshop/donate). Bilingual EN+HI. /impact page now renders this section under PageHeader spacing; Home shows ImpactMetrics then this section. Verified by testing agent (iteration_3.json): 100% frontend pass, zero console errors, mobile 390px no overflow, count-ups + region interactions + bilingual all working. a11y: region buttons given aria-labels.
- Impact revert to pink + numbers move (2026-08-21): Per user request — (1) reverted the impact map section from the editorial ivory/burgundy/gold back to the site's PINK UI (rutuja tokens: light-pink #F0DEE6 map fill, pink #C82B62 markers/journey lines, pink story card); removed gold block-print motifs. (2) Moved the "Documented field activity" story card to sit BELOW the "Select a region" selector in the right column. (3) Changed the ImpactMetrics "Impact" section (was ambassador 20+/700+/6) to the Doll Pilot Project numbers (2,924 / 38 / 13 / 11 May–31 Jul 2026 / 30–60 min) and REMOVED the now-duplicate National Pilot strip from the map section (single source). /impact page now renders ImpactMetrics (pilot numbers) + ImpactMap.
- Ambassadors map (2026-08-21): Per user's "Dignity Dialogue 2026" poster — repurposed the Impact map into the AMBASSADORS map. Replaced the 6 documented-region pins with the 12 numbered Dignified Menstruation Ambassadors across India & Nepal (data/impactMap.js AMBASSADORS with n + projected x/y; coords reconstructed via verified linear projection since /tmp was cleared). Pink numbered pins (1–12) with pulse + staggered drop-in; selector list of 12 (number badge + name + location); detail card shows number + role + name + location. Names kept in Latin script both languages; locations + chrome bilingual (content.js impactMap.eyebrow='From across India & Nepal', title='Our National Dignified Menstruation Ambassadors.', roleLabel, tapHint, ambassadors[12], plus pilot + closing retained). Nepal pin (#1 Lalbandi) sits near the northern border (India-only outline). Existing "Our Ambassadors" cards section left untouched. Verified via screenshots desktop + mobile (390px, 0 overflow), 12 pins + selection + detail all working.
